...
- Station 61 capture recorded on `feature/station61-legacy-capture` at `migration/legacy/station61/capture_20251219_185854Z` (capturedAt 2025-12-19T18:58:54Z UTC); raw DogTabs files preserved byte-for-byte with manifest.json + checksums.sha256.
- CI guard (`tools/ci/legacyStation61Guard.js`, lint workflow) blocks edits to existing `capture_*`; local run used env-fed status output (Node child_process blocked here) → pass.
...
- `agents.md` referenced in README is missing.

## Next Action

- Commit + PR for Station 61 branch; keep the capture directory immutable going forward.
