...
**Station 32 – Connect Kurse ↔ Trainer:** Branch `feature/station31-hunde-kurse` (Fortsetzung). Status: fertig implementiert, PR noch offen. Scope: Trainer-FK-Enforcement in Kurse (UI + API), Trainerkarten/-links in Kurs-Detail/Liste, Reverse-Liste “Kurse dieses Trainers”, Delete-Guard für zugewiesene Trainer, einheitliche Aktionen-Buttons. Tests: `pnpm lint` ✅, `pnpm vitest run` ✅, `pnpm build` ✅. Nächster Schritt: PR für Station 32 gegen `main` erstellen/prüfen.
